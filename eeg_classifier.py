...
if __name__ == "__main__":
    ...
    # main()
    # print(time.asctime(time.localtime(time.time())))
    inputs = torch.zeros((1, 1, 64, 200))
    model = Conformer(emb_size=40)
    _, outs = model(inputs)
    print(outs.shape)
    _1dNF, _2dNF, pre, post, dataset_table = get_eeg_subjects()
    ...
    img = torch.from_numpy(train_e_)
    label = torch.from_numpy(train_l_)

    dataset = torch.utils.data.TensorDataset(img, label)
    dataloader = torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, drop_last=True)

    test_data = torch.from_numpy(test_e_)
    test_label = torch.from_numpy(test_l_)
    ...
    for e in range(n_epochs):
        ...
        if (e + 1) % 10 == 0:
            ...
            # label = Variable(label.cuda().type(LongTensor))
                img = Variable(img.type(Tensor))
                test_label = Variable(label.type(LongTensor))
                Tok, Cls = model(img)
