...
def get_individuals_paths_01(path_fmri=os.environ['EEG_FMRI']+'/datasets/01/fMRI/', resolution_factor = 5, number_individuals=10):
    ...
    for c in content:
      ...
      if '.ipy' in c:
        ...
        print("appended")
        file_individuals.append(c)

    file_individuals = sorted(file_individuals)
...
def get_individuals_paths_10(path_fmri=os.environ['EEG_FMRI']+'/datasets/02/fMRI/', resolution_factor = 5, number_individuals=1):
    print("Inside 10")
    fmri_individuals = []
    content = listdir(path_fmri)
    file_individuals = []
    for c in content:
      ...
      if '.ipy' in c:
        ...
        file_individuals.append(c)
    file_individuals = sorted(file_individuals)
    target_shape = image.load_img(path_fmri + file_individuals[0] + '/task-1dNF_run-01_bold_64_64_30.nii.gz').shape
    target_shape = (int(target_shape[0]/resolution_factor), 
                    int(target_shape[1]/resolution_factor), 
                    int(target_shape[2]/resolution_factor))
    ...
    for i in range(number_individuals):
        ...
        fmri_file = '/task-1dNF_run-01_bold_64_64_30.nii.gz'
...
def get_individuals_paths_05(path_fmri=media_directory+dataset_05+"/", number_individuals=None, task="MIpost", downsample=True, downsample_shape=(64,64,32), resolution_factor=None):
