...
n_individuals_train_10 = 2
n_individuals_test_10 = 1
...
def load_data(instances, task, raw_eeg=False, n_voxels=None, bold_shift=3, n_partitions=16, by_partitions=True, partition_length=None, f_resample=2, mutate_bands=False, eeg_limit=False, eeg_f_limit=134, fmri_resolution_factor=4, standardize_eeg=True, standardize_fmri=True, ind_volume_fit=True, iqr_outlier=True, roi=None, roi_ica_components=None, dataset="05"):
...
def get_data(individuals, raw_eeg=False, raw_eeg_resample=False, task="MIpost", eeg_resample=2.160, bold_shift=3, n_partitions=16, by_partitions=True, partition_length=None, n_voxels=None, TR=2.160, f_resample=2, mutate_bands=False, eeg_limit=False, eeg_f_limit=134, fmri_resolution_factor=5, standardize_eeg=True, standardize_fmri=True, ind_volume_fit=True, iqr_outlier=True, dataset="05"):
    TR = 1/TR
    assert task in ["1dNF_run", "2dNF_run",  "MIpost", "MIpre"]
    X = []
    y = []
    fmri_scalers = []
    ...
    individuals_imgs = getattr(fmri_utils, "get_individuals_paths_"+dataset)(task=task, resolution_factor=fmri_resolution_factor, number_individuals=len(individuals))
